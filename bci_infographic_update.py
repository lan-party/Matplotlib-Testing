...
for a in range(0, len(resp)-1):
    ...
    if companyCountry not in countries:
        countries.update({companyCountry: {'value': 0}})
    countries[companyCountry]['value'] += 1
    ...
    for key in neuroimagingTechniques:
        if key in neuroimagingTechnique:
            ...
            neuroimagingTechniques.update({key: 1})
    ...
    for key in neurostimulationTechniques:
        if key in neurostimulationTechnique:
            ...
            neurostimulationTechniques.update({key: 1})
...
sub6 = fig.add_subplot(4,1,4,projection=ccrs.PlateCarree())
fig.tight_layout()
...
sub4.bar(list(neuroimagingTechniques.keys()), list(neuroimagingTechniques.values()))
